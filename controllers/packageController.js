...
const getPublicPackages = async (req, res) => {
  try {
    ...
    const packages = await Package.find({ business: business._id, isActive: true }).sort({ sortOrder: 1, createdAt: 1 });
    res.status(200).json({ success: true, count: packages.length, packages });
  } catch (err) {
    res.status(500).json({ success: false, message: "Failed to fetch packages", error: err.message });
  }
...
const createPackage = async (req, res) => {
  try {
    const { business, title, subtitle, therapyCount, price, discountPrice, isFeatured, sortOrder } = req.body;

    if (!business || !title || !price) {
      return res.status(400).json({ success: false, message: "Business, title and price are required" });
    }
    ...
    const pkg = await Package.create({
      ...
      title,
      subtitle,
      therapyCount: therapyCount || null,
      price,
...
const updatePackage = async (req, res) => {
  try {
    ...
    const fields = ["title", "subtitle", "therapyCount", "price", "discountPrice", "isFeatured", "isActive", "sortOrder"];
    fields.forEach((field) => {
      if (req.body[field] !== undefined) pkg[field] = req.body[field];
    });
